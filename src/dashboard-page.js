...
function buildPasserbyCard(kpi, index, dbResult) {
  const camLabel = `CAM-${String(index + 1).padStart(2, '0')}`;
  const displayTitle = kpi.label || KPI_LABELS[kpi.type] || kpi.type.toUpperCase();
...
function buildBillingCard(kpi, index, dbResult) {
    ...
    <div class="kpi-body">
      ...
      <div class="kpi-right">
        ...
        <div class="billing-queue-badge bq-low">
          <span class="bq-status">Queue Size &lt; 2</span>
        </div>
  ...
  const updateForTime = () => {
    ...
    const [queueClass, queueLabel] = queueSize < 2 ? ['bq-low', 'Queue Size < 2'] : queueSize <= 3 ? ['bq-medium', 'Queue Size 2–3'] : ['bq-high', 'Queue Size > 3'];
...
export async function renderDashboard(appEl, config) {
  ...
  config.kpis.forEach((kpi, i) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      if (kpi.type === 'footfall') {
        card = buildFootfallCard(kpi, i, dbResult);
      } else if (kpi.type === 'passerby') {
        card = buildPasserbyCard(kpi, i, dbResult);
