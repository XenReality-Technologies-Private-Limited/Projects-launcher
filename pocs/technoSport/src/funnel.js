...
const STAGES = [
  { key: 'pb',      label: 'Passerby',           sublabel: 'Outside Store',        color: '#2E3192', icon: '👥' },
  { key: 'gf',      label: 'Ground Floor Entry', sublabel: 'Store Visitors',       color: '#00AEEF', icon: '🚶' },
  { key: 'billing', label: 'Billing Counter',    sublabel: 'Completed Transaction', color: '#8B5CF6', icon: '🛒' },
];
...
export function updateFunnel(container, frame) {
  const { pbIn, gfIn, billingCustomers } = frame;
  const max = Math.max(pbIn, 1);

  const data = {
    pb:      { count: pbIn,              pctDen: pbIn,  pctNum: gfIn             },
    gf:      { count: gfIn,              pctDen: gfIn,  pctNum: billingCustomers },
    billing: { count: billingCustomers,  pctDen: null,  pctNum: null             },
  };
