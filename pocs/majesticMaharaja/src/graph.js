...
export class TimeSeriesGraph {
  ...
  render() {
    ...
    const paddingLeft = 8;
    const paddingRight = this.showLiveCount ? 40 : 8;
    const paddingTop = 8;
  ...
  render() {
    ...
    this.series.forEach(({ values, color, label }) => {
      ...
      if (this.showLiveCount && windowValues.length > 0) {
        ...
        labelsToDraw.push({ liveCount, color, y: lastY });
      }
    });
    ...
    if (labelsToDraw.length > 0) {
      ...
      // Push labels down to maintain minSpacing
      for (let i = 1; i < labelsToDraw.length; i++) {
        if (labelsToDraw[i].y - labelsToDraw[i - 1].y < minSpacing) {
          labelsToDraw[i].y = labelsToDraw[i - 1].y + minSpacing;
        }
      }
      
      // If we pushed labels too far down, shift the entire stack up
      const maxY = height - paddingBottom - 6;
      const overflow = labelsToDraw[labelsToDraw.length - 1].y - maxY;
      if (overflow > 0) {
        for (let i = 0; i < labelsToDraw.length; i++) {
          labelsToDraw[i].y -= overflow;
        }
      }

      labelsToDraw.forEach(({ liveCount, color, y }) => {
        ctx.fillStyle = color;
        ctx.font = 'bold 13px system-ui, sans-serif';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.fillText(String(liveCount), playheadX + 5, y);
      });
    }
