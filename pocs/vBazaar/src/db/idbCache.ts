// IndexedDB cache for merged per-store historical SQLite bytes.
...
const IDB_VERSION = 3; // v3 busts any caches from the old static site (which used v2)
...
export async function idbSet(key: string, value: Uint8Array): Promise<void> {
  try {
    ...
    // Quota exceeded or IDB unavailable — log and continue (cache miss on next load)
